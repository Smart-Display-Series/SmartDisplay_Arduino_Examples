...
// DisplaySSD1327_128x128_SPI display(8, { -1, 4, 7, 0, -1, -1 }); // Display SPI pin configuration for old OLED
...
int16_t MultiStateValue = 0;  //MultiState value store
...
unsigned char stmp[8] = { 1, 0, 0, 0, 0, 0, 0, 0 };   //Indicator (0x70C)array data Bytes
unsigned char stmp1[8] = { 1, 0, 0, 0, 0, 0, 0, 0 };  //Multistate (0x716)array data Bytes
unsigned char stmp2[8] = { 0, 2, 0, 0, 0, 0, 0, 0 };  //Circle Progress (0x70B)array data Bytes
unsigned char stmp3[8] = { 0, 1, 0, 0, 0, 0, 0, 0 };  //Image Progress (0x70E)array data Bytes
unsigned char stmp4[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };  //Gauge (0x702)array data Bytes
unsigned char stmp5[8] = { 0, 2, 0, 0, 0, 0, 0, 0 };  //Temperature (0x708)array data Bytes
unsigned char stmp6[8] = { 0, 1, 0, 0, 0, 0, 0, 0 };  //Battery (0x70A)array data Bytes
unsigned char stmp7[8] = { 0, 2, 0, 0, 0, 0, 0, 0 };  //VerticalSlider (0x706)array data Bytes
unsigned char stmp8[8] = { 0, 1, 0, 0, 0, 0, 0, 0 };  //HorizontalSlider (0x707)array data Bytes
unsigned char stmp9[8] = { 0, 2, 0, 0, 0, 0, 0, 0 };  //FixedWidget (0x713)array data Bytes
...
void waiteResponse()
{
  ...
  // After sending out a message, waiting for the smart display to respond before sending the next one 
  // still resulted in a display freeze. 
  // 
  // Short-term strategy : increased the delay time to 200 mSec, no freeze. 
  // Long-term strategy  : need to use the ST Link to enter debug mode on the display 
  //                       to observe the root cause of this issue.
...
void setup() {
  ...
  while (CAN_OK != CAN.begin(CAN_250KBPS)) {  // init can bus : baudrate = 250k
    ...
    CAN.setMode(MODE_NORMAL); // 設定為正常模式，開始接收訊息
...
void loop()
 {
  ...
  if (nowPage == 1){
    ...
    if( ImageProgressValue!= LastImageProgressValue ) 
    {
      LastImageProgressValue = ImageProgressValue;
      stmp3[0] = ImageProgressValue;
      CAN.sendMsgBuf(0x70E, 0, 2, stmp3);   // send data of the ImageProgress: id = 0x70E, byte ext, data len = 8, stmp3: data buf    
    }    
  }
  ...
  if (nowPage == 2){    
    if( GaugeValue != LastGaugeValue ) 
    {
      ...
      stmp4[0] = GaugeValue;     
      CAN.sendMsgBuf(0x702, 0, 7, stmp4);    // send data of the Gauge: id = 0x702, byte ext, data len = 8, stmp4: data buf
    }
  ...
  // CAN.sendMsgBuf(0x70C, 0, 8, stmp);    // send data of the Indicator: id = 0x70C, byte ext, data len = 8, stmp: data buf
  // CAN.sendMsgBuf(0x713, 0, 8, stmp9);   // send data of the FixedWidget: id = 0x70E, byte ext, data len = 8, stmp1: data buf
